...
\begin{frame}{}
	\begin{block}{Konkrete Forderungen des Business}
		...
		\end{itemize}
		\onslide<5->{\vskip3mm --- Manager eines Logistik-Dienstleisters}
...
\begin{frame}{}
	\begin{block}{SBT -- Scala build tool}
		\begin{itemize}
			...
			\item<4->Unterstützt interaktiven REPL (Read eval print loop)-Modus
			\item<5->Inkrementelles Kompilieren -- scalac ist langsam
			\item<6->Konfiguration per Scala-DSL
			\item<7->scalac und scala funktionieren auch
		\end{itemize}
	\end{block}
	\note{}
\end{frame}
...
\subsubsection*{PRÄSENTATION - Scratchpad}
...
\begin{frame}{}
	\begin{block}{Collection-Struktur}
		\begin{itemize}
			\item<2->Immutable und Mutable-Varianten
			\item<3->
			\item<4->
		\end{itemize}
	\end{block}
...
\begin{frame}[fragile]{}
	\begin{block}{Vector statt ArrayList}
		\begin{itemize}
			\item<2->Vector (scala.collection.immutable.Vector)
			\item<3->Immutable \emph{und} schnell
		\end{itemize}
		\end{block}
	\note{}
\end{frame}
...
\subsubsection{PRÄSENTATION - Vererbung}
\plain{Vererbung}
...
% Todo: Werden vars häufiger ausgewertet?
\begin{frame}[fragile]{}
\begin{block}{Lazy val}
		\scriptsize
		\onslide<2->
  \begin{lstlisting}
lazy val 
	...
	\normalsize
	\begin{itemize}
			\item<2->Auswertung bei erstem Aufruf
			\item<3->Einmalig
			\item<4->"`Memorized def"'
		\end{itemize}
	\end{block}
	\note{}
\end{frame}
