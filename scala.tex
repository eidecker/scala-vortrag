...
\begin{frame}{}
  \begin{quote}
	...
	\end{quote}
	\note{
Ich möchte heute über Scala reden. Eigentlich will ich aber nicht nur über Scala reden.  Ich möchte nicht Scala verkaufen, sondern etwas v�llig anderes. Scala ist spannend - f�r Nerds, die des Geldes wegen Java benutzen, denen aber mittlerweile Oracles Update-Politik nicht mehr gef�llt. 
...
\begin{frame}<presentation:0>{}
	\begin{block}{�ber mich}
		\begin{itemize}
			\item<2->Softwareentwickler
			\item<3->Enterprise-gesch�digt
			\item<4->Java-gepr�gt
		\end{itemize}
	\end{block}
	\note{Erz�hlen, wer ich bin, was ich mache und warum das meine Sicht auf die IT-Welt und Scala massiv beeinflusst}
\end{frame}

\begin{frame}{}
	\setbeamertemplate{section in toc}[sections unnumbered]
	\begin{block}{Wor�ber will ich reden?}
		\vskip2mm
		\onslide<1->{\tableofcontents[pausesections,pausesubsections,subsubsectionstyle=hide]} %[hideallsubsections]}
	\end{block}
	\note{Ich will nicht nur �ber Scala reden. Da ich netterweise als erster Redner einer neuen Veranstaltung eingeladen wurde, m�chte ich mir die Freiheit nehmen, etwas grunds�tzlicher �ber die Softwareentwicklung zu reden.
	
	Ich glaube, dass sich gerade -- oder auch schon seit einigen Jahren -- etwas Grundlegendes ver�ndert. Die IT bekommt eine neue Rolle und auch viel mehr Bedeutung und Verantwortung. Diese Zukunft m�ssen wir gestalten. In Unternehmen. Als Unternehmen.
...
\begin{frame}{}
	...
	\note{Software Engineering. Passt der Begriff? Ich denke, dies impliziert zu stark, dass wir ein Problem nur lange genug durchdenken, exakt definieren und dann eine gute L�sung bauen m�ssen. Wom�glich verstehe ich den Beruf des Ingenieurs falsch, aber ich glaube, wir haben gesehen, dass Software agil gebaut werden sollte, dass wir nicht zu Beginn eines Projektes alle Anforderungen exakt spezifizieren k�nnen. Und: Wir k�nnen kein System bauen, das hunderte Jahre stabil steht, wie eine Kirche oder Br�cke. Wir m�ssen es st�ndig pflegen und verbessern, damit es n�tzlich bleibt.
...
\begin{frame}{}
	\begin{block}{Agiles Manifest (2001)}
		% Todo: Ausf�hrlicher, die Punkte sind wichter als...
		\begin{itemize}
			...
			\item<5->Reagieren auf Ver�nderung mehr als das Befolgen eines Plans
		\end{itemize}
	\end{block}
\end{frame}
...
Im Grunde geht es �berall darum, gemeinsam Software zu entwickeln, dabei die Bed�rnisse des Kunden aktiv zu erforschen und m�glichst zuverl�ssige und n�tzliche Software zu erstellen.
}

\begin{frame}{}
	\only<1>{\Large\textbf{Nur Manifeste?}}
	\only<2>{\Large\textbf{Realit�tsabgleich}}
	\note{Erst einmal nur Manifeste. Sie stammen jedoch von Praktikern. Also schauen wir mal in die aktuelle Welt. Ich denke n�mlich, die Anforderungen und die Rolle der IT �ndern sich gerade dramatisch.
...
\begin{frame}{}
	...
	\only<3>{\Large\textbf{Mit welchem Gesch�ftsmodell?}}
	\note{Vielleicht: Uber l�sst von Google Autos bauen, nutzt deren Karten und bietet selbst fahrende Taxis?
	
	Ich habe die Autos als Beispiel verwendet, weil sie bei uns Deutschen so tief verwurzelt sind. Als ich Jugendlicher war, wollte/musste fast jeder einen F�hrerschein haben. Die Automobilindustrie ist m�chtig. Das nicht vorhandene Tempolimit auf Autobahnen ist uns heilig. F�r andere Branchen gilt sicher �hnliches.
...
% Todo: Raus, da Hellmann-Interna? Stattdessen �ffentliche Quelle suchen
\begin{frame}{}
	\begin{block}{Konkrete Forderungen des Business}
		\begin{itemize}
			\item<2->Stabilit�t und Resilienz
			\note<2>{Sorgen sie durch ihre Arbeit daf�r, dass Systeme Kriterien der Stabilit�t und Resilienz erf�llen.
			Seien sie "`dran"' an den Problemen, antizipieren sie sie, schaffen sie Fall-Back-L�sungen -- auch an Wochenenden.
			
			}
			\item<3->Wertbeitrag
			\note<3>{IT entwickelt sich zunehmend zu einem eigenen Produktionsfaktor (4-Sektor-Hypothese).
			Seien sie ein verl�sslicher Partner f�r das Business, nehmen sie die Business-Seite auch an die Hand, helfen und unterst�tzen sie, leisten sie einen wahrgenommenen Wertbeitrag.[...]
			
			}
			\item<4->Businesstreiber
			\note<4>{Die Entwicklung der IT treibt zunehmend die Business-Seite und ver�ndert Gesch�ftsmodelle.
			Seien sie an der Front und treiben sie das Business, entwickeln sie neue Ideen, forschen sie nach neuen L�sungen (Beispiele: Uber, 3D-Printing, CoLo21, ...
...
\begin{frame}{}
	...
	\note{Passt das? Haben wir �hnliche Herausforderungen? Haben wir diese schon gel�st?
...
\begin{frame}{}
	\begin{block}{Eigenschaften}
		\begin{itemize}
			...
			\note<2>{Keine Primitive, wie in Java. Ein sch�nes Beispiel: Dort hat man per Autoboxing Komplexit�t eingef�hrt statt Sch�nheit. NullPointer bei Primitiven anyone?}
			\item<3->Funktional -- First class functions
			\item<4->Statisch typisiert mit Type Inference
			\note<4>{Aber Type Inference, wenn gew�nscht}
			\item<5->Immutable wird bevorzugt
			\item<6->Gewohnte Syntax ("`Java ohne Semikolon"')
			\item<7->Ausdrucksstark (APIs/DSLs)
			\note<7>{Bei Java bricht man sich schon die H�nde, will man ein Builder-Pattern rekursiv umsetzen oder eine fluent API}
...
\begin{frame}{}
	...
	\begin{itemize}
		\item<2->Produktivit�tssteigerung
		\item<3->H�here Codequalit�t
		\item<4->Mehr Spa�
		\note<4>{REPL, habe ich mir f�r Java schon immer gew�nscht. Wer hat keine Testklasse, um mal etwas auszuprobieren?}
		\item[]<5->\alert{durch}
		\item<5->Weniger Code
		\item<6->H�heres Abstraktionsniveau
		\item<7->Skalierbarkeit
	\end{itemize}
	\end{block}
\end{frame}
\note{Passt in unseren Entwicklungsprozess. Leichte �nderungen an Deployment etc.
...
\begin{frame}{}
	\begin{block}{Scala und die Java-Plattform}
		\begin{itemize}
			\item<2->Java-Bytecode, l�uft auf JVM
			\item<3->Java-Bibliotheken nutzbar
			\item<4->�hnliches Toolset, oft wiederverwendbar
...
\begin{frame}{}
	\begin{block}{IDE-Unterst�tzung}
		\begin{itemize}
			\item<2->Die �blichen Verd�chtigen 
			\item<3->Ausgewachsene IDEs: IntelliJ, Eclipse, Netbeans
			\item<4->Plugins f�r Editoren: Emacs, vi, ...
...
\begin{frame}{}
	\begin{block}{Continuous Integration}
		\begin{itemize}
			\item<2->SBT-Plugins f�r Jenkins/Bamboo
			\item<3->Unit-Tests: Scalatest
			\item<4->Statische Codeanalyse: ScalaStyle
			\item<5->Tools f�r die JVM funktionieren
		\end{itemize}
	\end{block}
...
\begin{frame}{}
	\begin{block}{Scala in Code-Beispielen}
		\begin{itemize}
			\item<2->Ausgew�hlte Bereiche -- Scala ist komplex
		...
		\end{itemize}
	\end{block}
	\note{Komplexit�t erinnert mich an die deutsche Grammatik. Man beherrscht sie irgendwann recht gut. Soll man aber grammatikalische Figuren angeben, ist man schnell am Ende. Au�er man lernt die Details.
	
	}
\end{frame}

\subsubsection*{Einf�hrendes Beispiel und Gegenbeispiel DONE}
...
\begin{frame}[fragile]{}
	...
	\note{Wo ist die Businesslogik versteckt? Ich gebe zu, das wird nur ein Beispiel, aber es zeigt symptomatisch eine Schw�che von Java: 
Java ist leicht verst�ndlich, �nderungen werden aber immer non-breaking eingef�hrt und f�hlen sich oft deplatziert an.
Scala hat den Vorteil, neuer zu sein und kennt die Schmerzen der Java-Entwickler. Daher sind viele Dinge eingebaut, die man sich schon lange w�nscht 
oder Konzepte, die man eigentlich schon immer umsetzen wollte, wenn es denn nicht so l�stig w�re.
(Immutable Objects, Lambdas, Generics, checked Exceptions als Beispiel)

Ein besonders l�stiges Nicht-Feature-Feature sind in meinen Augen JavaBeans, wie wir sie gerade gesehen haben. Leicht verst�ndlich, aber strikte Richtlinien durch Konvention. Und wehe, dein Framework mag die Namen Deiner Getter und Setter nicht.
...
case class Person(firstName:String, lastName:String)
	\end{lstlisting}
\end{block}
\note{Ja, nur ein Beispiel, ein besonders krasses. Aber dennoch ein allt�gliches.}
\end{frame}
...
def quickSort[A <% Ordered[A]](xs: List[A]): List[A] = xs match {
  ...
  case y :: ys => ys partition (_ <= y) match {
	...
	\end{lstlisting}
\end{block}
\note{Man sieht: Ausdrucksstark, aber auch nicht mehr so straight forward wie Java (vor 8). Scala ist beides: Sehr einfach und fast beliebig komplex. Und dieser Quicksort ist noch lange nicht das Ende der M�glichkeiten.
...
\end{lstlisting}
\end{block}
\onslide<4->\textbf{Compile-Fehler f�r k, da immutable}
\onslide<5->
...
def f(i: Int) = {
	...
	\end{lstlisting}
\end{block}
\note{Hier mal eine Methode. Mit = definiert. Methoden k�nnen mehr als in Java, dazu sp�ter.
Die Klammern kann man sich sparen, wenn 
Typen kann man angeben, muss es aber nicht, so lange der Compiler sie ermitteln kann. 
Bei �ffentlichen Methoden sollte man es aber tun, dann hat der Nutzer auch eine reelle Chance.
...
\note{i ist val, also immutable. Immer, var nicht m�glich. Letzter Ausdruck ist return}
\end{frame}
...
def f() = {
  if (something)
	...
	\end{lstlisting}
\end{block}
\onslide<3->{\textbf{Letzte Anweisung wird zur�ckgegeben, impliziter Typ String.}}
\note{
Kein return, letzte Anweisung wird zur�ckgegeben. Alles hat einen Typen!
Die Methode ist statisch typisiert! Der Compiler kann den Typen der R�ckgabe ermitteln: String -- Type Inference
Wenn wir wollen, k�nnen wir ihn deklarieren. Bei �ffentlichen Methoden sollte man das auch tun.
...
def print(s: String): Unit = println(s)
	\end{lstlisting}
\end{block}
\note{Wollen wir mal wirklich nichts zur�ckgeben, so existiert der Typ Unit. Sinnvoll bei Methoden, die ausschlie�lich Seiteneffekte haben. Sollte also selten sein.
...
% Pr�sentation: Alles ist ein Typ im Scratchpad. Das ist aus meiner Sicht praktisch
\plain{Worksheet}
...
\end{lstlisting}
\note<7>{
== pr�ft auf fachliche Gleichheit. eq die Objektreferenzen.
...
\end{lstlisting}
\end{block}
\note{Funktionen k�nnen als Parameter �bergeben werden. Auch die Funktion ist ein Objekt.
Kennt man in Java mittlerweile auch, wirkt aber aus meiner Sicht nicht so sch�n integriert.
...
\end{block}
\note{Wir k�nnen Funktionen auch wie Variablen verwenden und �bergeben. Es sind first class citizens. Und das ist m�chtig. Das sehen wir schon in Java 8, aber in Scala war es schon immer und wird bereits flei�ig in APIs genutzt. Das macht es f�r Programmierer, die keinen funktionalen Background haben, leider auch nicht einfacher. Methoden sind �brigens keine Objekte (Methode per def, Funktion per val deklariert.)
Methode in Funktion umwandeln
def addOne(n: Int) = n+1
val f = addOne \_

addOne kann dennoch �bergeben werden, da Compiler die Arbeit �bernimmt.
...
val t2 = new Time
	\end{lstlisting}
\end{block}
\note{Der Standardkonstruktor f�r alle Parameter ist direkt im Methodenrumpf. Hilfskonstruktoren sind m�glich.
val Standard, Felder sind damit verf�gbar. Var m�glich, dann Setzen m�glich?
...
val t2 = new Time(minutes = 13)
	\end{lstlisting}
\end{block}
\note{Damit werden Argumentnamen aber auch API-Bestandteil und d�rfen nicht ge�ndert werden.}
\end{frame}
...
val t = Time.fromMimutes(100)
	\end{lstlisting}
\end{block}
\note{Companion Objects sind Singleton Objects. Sie existieren nur ein mal. "`Ersatz"' f�r static, das es nicht gibt. N�tzlich f�r Factory-Methoden.
Companion Objects (selber Name, selbes Paket, selbe Datei wie Klasse) k�nnen auf private Elemente der Klasse zugreifen.

Gilt auch f�r Traits.
...
val sohn = ich.copy(vorname = "Nils")
	\end{lstlisting}
\end{block}
\note{Case classes sind JavaBeans in sch�n. Nehmen die �blichen Dinge ab, die sowieso die IDE macht. Ideal als DTO, man kann/sollte aber nicht von ihnen erben. H�bscher "`Konstruktor"', Copy-Konstruktor und toString
...
\end{itemize}
\end{block}
\note{�hnlich Java in den Grundz�gen. Mit Traits Mehrfachvererbung m�glich und funktionalerer Ansatz. Dadurch h�ufig kleinere Funktionsbl�cke in Traits. Java 8 bietet mit default-Methoden in Interfaces �hnliches.
...
\begin{frame}{}
	\begin{block}{Traits -- Einschr�nkungen}
		...
		\end{itemize}
	\end{block}
	\note{Aufl�sen von Problemen durch Linearisierung der Vererbungshierarchie. Es kann zu Problemen kommen. Kann und m�chte ich in dieser kurzen Zeit aber nicht motivieren. Ist in der Realit�t aber wohl selten ein Problem.
...
% Verschiedene Matching-M�glichkeiten darstellen
...
number match {
  ...
  case _ => "some other number"
	...
	\end{lstlisting}
	\end{block}
	\note{Sehr m�chtiges Feature. Code wirkt auf einmal sinnvoll. Java-Case auf Steroids ist noch zu schwach. Vielf�ltige Pattern m�glich. Dabei Aufspalten von Listen m�glich, siehe Quicksort.
...
for (person <- List(alice, bob, charlie)) {
	...
	\end{lstlisting}
\end{block}
\note{Wir k�nnen Attribute matchen und diese sogar an Variablen binden und verwenden.
...
Da f�hlt sich der Compiler-Entwickler foch schon fast abgeholt.
...
def factorialTail(n: Int): Int = {
  ...
  factorialAcc(1, n)
	...
	\end{lstlisting}
\end{block}
\note{Es m�ssen keine Ergebnisse auf dem Stack zwischengespeichert werden. Optimierung durch den Compiler. Warnung, wenn nicht tail recursive.
...
\end{lstlisting}
\end{block}
\note{Haben wir beim Pattern Matching gesehen. Tupel k�nnen aber auch n�tzlich sein, wenn man mal kein neues Objekt erstellen will. Oder, wenn man Maps verwendet. Oder, wenn man Funktionen als Parameter �bergibt. 
...
case class Time(hour:Int, minute:Int) {
  ...
  def minus(time: Time) = ???
	...
	\end{lstlisting}
\end{block}
\onslide<3->{Kompilierbar, aber nicht gef�hrlich.}
\note{Kompiliert, beim Aufruf aber Exception. Hilfreich, wenn man test first entwickelt. Dann jammert der Compiler nicht st�ndig.}
\end{frame}
...
\normalsize{\textbf{Erweitert API bestehender Klassen!}}
	\end{block}
	\note{Receiver: Objekte, auf denen Methoden aufgerufen werden. Hier gibt es die Methode nicht. Der Compiler sucht nach - in String. Wir m�ssen ihm eine Conversion geben, so dass er Time sieht, und damit - aufrufen kann.
	
	M�chtig. F�r DSLs. Sieht man aber kaum noch!}
\end{frame}
...
def factorial(n: Int): Int = {
  ...
  factorialAcc(1, n)
	...
	\end{lstlisting}
	\end{block}
	\note{Praktisch bspw. f�r rekursive Funktionen. Haben wir dort schon gesehen.}
\end{frame}
...
\begin{frame}{}
	 \begin{block}{Predef}
			 \begin{itemize}
				 \item<2->\texttt{java.lang} f�r Scala
...
\begin{frame}{}
	...
	\note{Insbesondere im Bereich der funktionalen Programmierung. Aber daf�r haben wir leider keine Zeit. Und ich kenne auch bwi weitem nicht alles.
...
150 LET ('v := 'v + 'burn * 10 / ('fuel + 'mass))
	\end{lstlisting}
\end{block}
\note{Ist nat�rlich Quatsch, zeigt aber, was geht. Und wir kommen auch gleich }
\end{frame}
...
"Creating a Time" should {
	...
	\end{lstlisting}
\end{block}
\note{DSLs sind sinnvoll, wenn sie besser zum Ausdruck bringen, was in der Dom�ne beschrieben wird.
...
\begin{frame}{}
	\begin{block}{Aktoren}
		\begin{itemize}
				\item<2->Nebenl�ufige Einheiten
				\item<3->Empfangen Nachrichten (Ereignisse)
				\item<4->Abarbeitung FIFO
				\item<5->Verhaltens�nderung
				\item<6->Asynchrone Kommunikation mit Aktoren
			\end{itemize}
	\end{block}
	\note{Kommen wir zu etwas wirklich Spannenden. Ich habe davon zugegeben kaum Ahnung, finde es aber so interessant, dass ich es heute erz�hlen muss!
...
Das ist �hnlich der reinen Objektorientierung, aber komplett asynchron. Die Idee ist �brigens alt (1973) und schon lange, z.B. in Erlang im Einsatz. Momentan passen die Probleme aber.
...
\begin{frame}{}
	\begin{block}{Akka}
		\begin{itemize}
			\item<2->Aktoren
			\note<2>{Klar, sonst h�tte es die vorherige Folie nicht gegeben.
			Actors give you:
...
Nachrichten mit Case-Klassen, einfacher geht es nicht. Und typsicher, was mir pers�nlich gut gef�llt.
...
class Pong extends Actor {
  def receive = {
	...
	\end{lstlisting}
\end{block}
\note{Und hier das Gegenst�ck:
Ein einfaches Beispiel, aber man sieht, wie hier Nachrichten ausgetauscht werden. Das soll hier aber keine Akka-Schulung sein, die ich auch nicht halten k�nnte, sondern
eine Idee geben, warum Scala geeignet sein k�nnte, Aktoren zu implementieren.
...
Ich kann mir nicht helfen, aber das liest sich f�r mich wie ein Business-Prozess. Den man beliebig zusammenstecken kann. Und er funktioniert dennoch, weil man in der Programmiersprache dasselbe macht wie in einem EPK oder BPMN-Modell. (Disclaimer: Ich habe es noch nicht viel weiter als bis zu diesem Niveau ausprobiert, aber Menschen, die es getan haben, stimmen darin �berein).
...
% Todo
\begin{frame}{}
	\begin{block}{Einsatzm�glichkeiten}
		\begin{itemize}
			\item<2->Live-Dom�nenmodell, automatisch berechnend
...
val route =
	...
	\end{overlayarea}
\end{block}
\note{Routen k�nnen auch in separaten Dateien (am besten in Traits) definiert und dann kombiniert werden.
...
\subsubsection*{PR�SENTATION Akka HTTP}
\plain{Akka HTTP}
...
\subsection*{Abschlie�ende Worte}
    ...
    \column{0.5\textwidth}
      ...
      \begin{block}{Vorteile}
		\begin{itemize}
			...
			\item<5->Verst�ndlich funktional
			\item<6->Java-�kosystem
			\item<7->Macht Spa�
    ...
    \column{0.5\textwidth}
      ...
      \begin{block}{Nachteile}
		\begin{itemize}
			...
			\note<11->{Ich glaube ja, und mittlerweile ist es auch keine Nische mehr. Wenn man aber darauf angewiesen ist, dass eine Technologie auch in 10 Jahren noch genauso vorhanden ist (was wahrscheinlich schlecht ist), dann muss man vielleicht den an sich schlechteren Mainstream w�hlen. Scala ist aber bei weitem nicht mehr esoterisch.}
			\item<12->Anzahl Entwicklungssklaven
			\note<12->{Es gibt eine Lernkurve, man braucht Entwickler, die Spa� daran haben und die Vorteile sehen wollen.}
			\item<13->Bin�rkompatibilit�t nicht in alle Ewigkeit
			\note<13->{Ist besser geworden, insbes. mit Java 8. Die Scala-Macher sind aber nicht so konservativ wie Oracle. Aber wozu das f�hrt, haben wir ja gesehen.}
		\end{itemize}
	\end{block}
  \end{columns}
	\note{}
    ...
    \column{0.5\textwidth}
      ...
      \begin{block}{Geeignet}
		\begin{itemize}
			...
			\item<6->Tests f�r Java-Code
			\item<7->Hoher funtkionaler Anteil
			\item<8->Java als Sprache abl�sen
		\end{itemize}
	\end{block}
    \column{0.5\textwidth}
	  \onslide<9->
...
\begin{frame}{}
  \begin{quote}
	...
	\end{quote}
	\note{Der Guardian ist der Inbegriff f�r den Technologiewandel in einer klassischen, beh�bigen Branche. Lange, bevor Springer wach geworden ist.}
\end{frame}

\begin{frame}{}
	\begin{block}{Literatur}
		\begin{itemize}
			\item<2->Heiko Seeberger: "`Durchstarten mit Scala. Tutorial f�r Einsteiger"'
			\note<2>{Sehr angenehmes Einsteigerbuch, das nur an der Oberfl�che kratzt, aber gen�gend zeigt, um arbeiten zu k�nnen.}
			\item<3->Martin Odersky: "`Programming in Scala"'
			\note<3>{Unglaublich detaillierte Vorstellung des Sch�pfers. Lest das nicht zuerst, es erschl�gt.}
			\item<4->Das Internet, nur ein Beispiel \texttt{https://twitter.github.io/scala\_school}
			% Todo: Mehr Referenzen, z.B. Pr�sentationen, Screencasts
		\end{itemize}
	\end{block}
	\note{
}
...
% Notes
% Notes k�nnen innerhalb von Folien definiert und mit Overlays spezifiziert werden. Dann werden sie entsprechend zum Folienteil angezeigt.
